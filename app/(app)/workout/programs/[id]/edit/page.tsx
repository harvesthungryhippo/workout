...
export default function EditProgramPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/workout/programs/${id}`).then((r) => r.ok ? r.json() : null),
      ...
      if (program) {
        ...
        setIsActive(program.active);
        setDays(program.days.map((d: { dayNumber: number; name: string; exercises: { exerciseId: string; exercise: { name: string }; order: number; sets: number; reps: string; restSeconds: number; notes: string | null }[] }) => ({
      ...
      if (exData) setExercises(exData.exercises ?? []);
    }).catch(console.error).finally(() => setLoading(false));
  }, [id]);

  function addDay() {
    setDays((prev) => [...prev, { dayNumber: prev.length + 1, name: `Day ${prev.length + 1}`, exercises: [] }]);
  }
  ...
  function addExerciseToDay(dayIndex: number, ex: Exercise) {
    setDays((prev) =>
      prev.map((d, i) =>
        i !== dayIndex ? d : {
          ...d,
          exercises: [...d.exercises, { exerciseId: ex.id, exerciseName: ex.name, order: d.exercises.length + 1, sets: 3, reps: "8-12", restSeconds: 90, notes: "" }],
        }
      )
    );
  }
  ...
  function updateExercise(dayIndex: number, exIndex: number, field: string, value: string | number) {
    setDays((prev) =>
      prev.map((d, i) =>
        i !== dayIndex ? d : { ...d, exercises: d.exercises.map((e, j) => (j !== exIndex ? e : { ...e, [field]: value })) }
      )
    );
  }
  ...
  function removeExercise(dayIndex: number, exIndex: number) {
    setDays((prev) =>
      prev.map((d, i) =>
        i !== dayIndex ? d : { ...d, exercises: d.exercises.filter((_, j) => j !== exIndex).map((e, j) => ({ ...e, order: j + 1 })) }
      )
    );
  }
  ...
  async function save() {
    ...
    if (res.ok) {
      ...
      router.push(`/workout/programs/${id}`);
    } else {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <Link href={`/workout/programs/${id}`} className="flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 mb-3">
        ...
        <Card key={dayIndex}>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <div className="flex items-center gap-3 flex-1">
              ...
              <Input value={day.name} onChange={(e) => updateDay(dayIndex, "name", e.target.value)} className="h-8 max-w-48" placeholder="e.g. Push" />
            </div>
          ...
          <CardContent className="pt-0 space-y-3">
            ...
            <details className="group">
              ...
              <div className="mt-2 max-h-48 overflow-y-auto border rounded-lg divide-y">
                {exercises.map((ex) => (
                  <button key={ex.id} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center justify-between" onClick={() => addExerciseToDay(dayIndex, ex)}>
                    <span>{ex.name}</span>
      ...
      <div className="flex justify-end gap-3 pb-8">
        <Link href={`/workout/programs/${id}`}><Button variant="outline">Cancel</Button></Link>
